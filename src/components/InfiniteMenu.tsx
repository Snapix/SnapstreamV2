...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <h2 className={`face-title font-display text-white ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title}</h2>

          <p className={`face-description text-zinc-300 ${isMoving ? 'inactive' : 'active'}`}> {activeItem.description}</p>

          <div onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'}`}>
            <p className="action-button-icon font-black flex items-center justify-center pt-1.5">&#x2197;</p>
          </div>
        </>
      )}
    </div>
